...
@torch.no_grad()
def DiffAugmentTest(image_size = 256, data = './data/0.jpg', types = [], batch_size = 10, rank = 10, rank = 0, nrow = 5):
    model = DummyModel()
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        if os.path.exists(file):
            ...
            iamges_augment = aug_wrapper(images = image_batch, prob = 1, types = types, detach = True)

            save_result = file_name + f'_augs{ext}'
            torchvision.utils.save_image(iamges_augment, save_result, nrow = nrow)
